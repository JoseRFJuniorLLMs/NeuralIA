...
- **Zoom com Ctrl+roda e pinça do touchpad (item 11, 8961c2e):** sobem e descem os mesmos degraus do Ctrl+= e do Ctrl+-, com o mesmo aviso; uma página que trata o gesto ela mesma fica com ele. Revisão: também por cima de iframes (prévia de artefato do Claude, canvas do Gemini, YouTube ou Maps embutidos), onde antes não fazia nada. Uma página consegue pedir degraus de zoom por esse caminho, e só isso.

### Testing
- Revisão da integração 2.1.8 (integrate/2.1.8), cada correção com o seu gate e prova de sabotagem: `the_window_buttons_stay_hidden_under_the_fullscreen_service_panel`, `hidden_caption_buttons_keep_their_place_under_the_raised_panel`, `the_home_panels_leave_the_window_buttons_reachable`, `a_window_over_the_panel_keeps_its_own_wheel`, `the_wheel_hook_only_redirects_when_the_panel_is_under_the_cursor`, `ctrl_wheel_over_a_frame_zooms_through_the_top_keymap`, `leaving_the_panel_fullscreen_undoes_only_what_the_panel_did`, `a_late_none_from_a_column_does_not_erase_another_hint`, `the_installer_window_paints_the_brand_on_the_tissue`, `the_main_window_is_created_with_the_project_icons`; `nothing_includes_the_brand_files_the_owner_deleted` passa a normalizar separadores e maiúsculas.
