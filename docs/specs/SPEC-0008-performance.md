...
Startup measurement alone cannot observe a lifecycle leak, so a second blocking gate (`scripts/measure-cycles.ps1`) drives comparator -> Home cycles. In CI the app exposes a test-only lifecycle probe containing its own live WebView-controller count; the gate requires exactly 3 in the comparator and 0 after every return to Home. It also fails on excessive working-set growth measured from the first post-Home warm sample, so the one-time cost of initializing WebView2 is not misclassified as a lifecycle leak. Process counting is intentionally not used because WebView2 may reuse renderer processes independently of controller lifetime.
