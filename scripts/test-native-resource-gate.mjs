...
const thresholds = [
  [/\[int\]\$MaxHandleGrowth\s*=/, 'MaxHandleGrowth'],
  [/\[int\]\$MaxThreadGrowth\s*=/, 'MaxThreadGrowth'],
  [/\[int\]\$MaxGdiGrowth\s*=/, 'MaxGdiGrowth'],
];

for (const [pattern, name] of thresholds) {
  assert.match(gate, pattern, name + ' must remain an explicit integer threshold parameter');
}
