...
/// ela prende UM atalho nas WebViews: o Ctrl+D dos favoritos (ambito
/// `Global`, no PR do seu comando -- regra C13), em cada hospedeiro com
/// teclado e com esse hospedeiro como origem. Nenhum dos outros atalhos
...
#[test]
fn accelerator_lookup_binds_only_ctrl_d_in_webviews() {
    ...
    // O Ctrl+D: preso em cada hospedeiro com teclado, com ELE como origem;
    // a tecla presa fica tratada sem repetir; a subida passa.
    for &host in &hosts {
        let down = accelerator_lookup(product_keymap(), host, press(ctrl_d));
        let held = accelerator_lookup(
            product_keymap(),
            host,
            AcceleratorInput {
                repeat: true,
        ...
        assert!(!up.handled && up.event.is_none(), "{host:?}");
        if host == WebViewHost::GmailMonitor {
            assert!(!down.handled && down.event.is_none());
            continue;
        }
        assert!(down.handled, "{host:?}");
        assert!(
            matches!(
                down.event,
                Some(UserEvent::RunCommandKey {
                    key: CommandId::Bookmark,
        ...
        assert!(held.handled && held.event.is_none(), "{host:?}");
    }
    // E e o unico atalho fora do ambito da janela.
...
fn accelerator_decision_table() {
    ...
    // Os sete da 2.2.0 e o Ctrl+D (Global, vale tambem la).
...
/// vira mapa, cada comando tem uma linha, o ambito da janela tem os
/// atalhos da 2.2.0 e o unico atalho novo e o Ctrl+D dos favoritos, no PR
/// do seu comando (regra C13).
#[test]
fn keymap_chords_are_unique_per_scope() {
    ...
    // A janela e a omnibox tem os da 2.2.0; o Ctrl+D dos favoritos e o
    // unico `Global`.
    ...
    for row in COMMANDS {
        for spec in row.chords {
            match spec.scope {
                KeyScope::Window => window.push((row.id, spec.chord)),
                KeyScope::Global => global.push((row.id, spec.chord)),
                other => panic!("{:?}: atalho no ambito {other:?}", row.id),
            }
        }
    }
    assert_eq!(global, [(CommandId::Bookmark, Chord::ctrl(b'D'))]);
    assert_eq!(
        window,
        [
...
fn the_window_and_the_omnibox_share_one_keymap() {
    ...
    // Os sete da 2.2.0 e o Ctrl+D dos favoritos (Global).
    assert_eq!(bound, 8);
                ...
                for label in [None, running, paused] {
                    ...
                    for index in 0..COMPARATOR_COLUMNS {
                        ...
                        for button in ColumnButton::ALL {
                            ...
                            // 1280 px para cima ha lugar para ela.
                            if button.optional() && logical_width < 1280.0 {
                                continue;
                            }
                            assert!(
...
fn existing_stores_have_a_declared_kind() {
    use crate::stores::{
        ADBLOCK_LIST_STORE, ADBLOCK_SETTINGS_STORE, APP_STORES, BOOKMARKS_STORE,
